...
const Home = () => {
  ...
  const [search, setSearch] = useState(() => {
    const saved = localStorage.getItem('yatri_search');
    return saved ? JSON.parse(saved) : { from: '', to: '', date: '', travelers: '1' };
  });
  const [packages, setPackages] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    localStorage.setItem('yatri_search', JSON.stringify(search));
  }, [search]);
      ...
      <section className="home-section">
        ...
        <div className="dest-grid">
          {destinations.map((dest) => (
            <div key={dest.id} className="dest-card" onClick={() => {
              const newSearch = { ...search, to: dest.name };
              setSearch(newSearch);
              navigate('/explore', { state: { initialSearch: newSearch } });
            }}>
